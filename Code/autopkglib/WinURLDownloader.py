...
"""See docstring for WinURLDownloader class"""
...
# Extract cab file:
# expand -R C:\Users\nmcspadden\Downloads\curl-7.60.0.cab -F:* "C:\Program Files\curl"
# Invoke curl:
# & 'C:\Program Files\curl\I386\CURL.EXE' --show-error --no-buffer --fail
# --dump-header - --speed-time 30 --location
# --url "https://download.mozilla.org/?product=firefox-stub&os=win&lang=en-US"
# --output "Firefox Installer.exe"
...
def setxattr(pathname, attr, content):
    """Set a named ADS on a file."""
    handler = pyads.ADS(pathname)
    handler.add_stream_from_string(attr, content)


class WinURLDownloader(Processor):
    ...
    input_variables = {
        ...
        "CURL_PATH": {
            "required": False,
            "default": "C:\Program Files\curl\i386\curl.exe",
            "description": "Path to curl binary. Defaults to C:\Program Files\curl\i386\curl.exe.",
        },
    }
    output_variables = {
        "pathname": {
